...
% La función ab4am4 resuelve un problema de valor inicial de la forma
% x'(t)=f(t,x(t)) en [t0,T]
% x(t0)=x0,
% con x0 en R^n, mediante el método de predicción-corrección con predictor Adams-Bashford de 4 pasos y corrector Adams-Moulton de 4 pasos.
%
% ENTRADA:
...
% x: matriz de ordenadas de la solución aproximada de tipo (N+1,n)
    ...
    [~, x] = rk3(f, [t(1),t(4)], x0, 3); %x1 con runge kutta orden 3
    x = x.';
    y = [f(t(3), x(:, 3)), f(t(2), x(:, 2)), f(t(1), x(:, 1)), [0;0]];
    for i = 1 : N-3
        y = [f(t(i + 3), x(:, i + 3)), y(:, 1), y(:, 2), y(:, 3)];        %f calculado con el x nuevo
        x(:, i + 4) = x(:, i + 3) + (h/24)*(55*y(:, 1) - 59*y(:, 2) + 37*y(:, 3) - 9*y(:, 4)); %x(i+4) con adam-bashford
        x(:, i + 4) = x(:, i + 3) + (h/720)*(251*f(t(i + 4), x(:, i + 4)) + 646*y(:, 1) - 264*y(:, 2) + 106*y(:, 3) - 19*y(:, 4)); %x(i+4) con adam-moulton
    end
    t = t(:);
    x = x.';
end